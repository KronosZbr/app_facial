  ...
  Future<void> _detectFaces() async {
    ...
    setState(() {
      if (faces.isNotEmpty) {
        _faceMessage = faces
            .map((face) {
              final happinessProb = face.smilingProbability ?? 0.0;
              String emotion = "Neutro";
              if (happinessProb > 0.7) {
                emotion = "Feliz";
              } else if (happinessProb < 0.3) {
                emotion = "Sério";
              }
              return "Rosto detectado: $emotion (Confiança: ${(happinessProb * 100).toStringAsFixed(1)}%)";
            })
            .join("\n");
      } else {
